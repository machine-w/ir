class AddPropertyOper < Mongoid::Migration
  def self.up
    ...
    pic.property_opers.create!(name: "原始大小",description: "图片原始尺寸显示",function_name:"origin",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| ("<a href='"+file.to_s+"'><img src='"+file.to_s+"' title='"+name+"' alt='"+name+"没有显示' class='user-img'></a>").html_safe }})
  	...
  	video.property_opers.create!(name: "原始播放",description: "嵌入视频播放",function_name:"origin",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"?random="+rand(999999).to_s+"' controls='controls' class='user-video'>你的浏览器不支持播放视频,<a href='"+file.to_s+"'>点击下载</a></video>" }})
  	video.property_opers.create!(name: "小尺寸播放",description: "嵌入视频播放",function_name:"small",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"?random="+rand(999999).to_s+"' controls='controls' class='small-video'>你的浏览器不支持播放视频,<a href='"+file.to_s+"'>点击下载</a></video>" }})
  	video.property_opers.create!(name: "中尺寸播放",description: "嵌入视频播放",function_name:"normal",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"?random="+rand(999999).to_s+"' controls='controls' class='normal-video'>你的浏览器不支持播放视频,<a href='"+file.to_s+"'>点击下载</a></video>" }})
  	video.property_opers.create!(name: "大尺寸播放",description: "嵌入视频播放",function_name:"big",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"?random="+rand(999999).to_s+"' controls='controls' class='big-video'>你的浏览器不支持播放视频,<a href='"+file.to_s+"'>点击下载</a></video>" }})
    ...
    music.property_opers.create!(name: "嵌入播放",description: "嵌入音频播放",function_name:"play",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<audio controls><source src='"+file.to_s+"'>你的浏览器不支持播放音频,<a href='"+file.to_s+"'>点击下载</a></audio>" }})
