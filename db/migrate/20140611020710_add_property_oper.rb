class AddPropertyOper < Mongoid::Migration
  def self.up
  	...
  	video.property_opers.create!(name: "原始播放",description: "嵌入视频播放",function_name:"origin",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"' controls='controls' class='user-video'>你的浏览器不支持播放视频</video>" }})
  	video.property_opers.create!(name: "小尺寸播放",description: "嵌入视频播放",function_name:"small",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"' controls='controls' class='small-video'>你的浏览器不支持播放视频</video>" }})
  	video.property_opers.create!(name: "中尺寸播放",description: "嵌入视频播放",function_name:"normal",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"' controls='controls' class='normal-video'>你的浏览器不支持播放视频</video>" }})
  	video.property_opers.create!(name: "大尺寸播放",description: "嵌入视频播放",function_name:"big",params: [],function_context: %Q{property_oper = lambda{ |name,file,size| "<video src='"+file.to_s+"' controls='controls' class='big-video'>你的浏览器不支持播放视频</video>" }})
